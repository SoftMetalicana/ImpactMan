namespace ImpactMan.Models.Static
{
    ...
    /// <summary>
    /// Represents a ground on the console.
    /// An object that does not move but has consequences when the player steps on.
    /// </summary>
    [MapObject(UnitConstants.GroundCsvKeyName)]
    public class Ground : Consequential
    {
        ...
        public Ground(int x, int y, string assetName, int bonusPoints) 
            : base(x, y, assetName, bonusPoints, ConsequentialConstants.GroundDistanceFromCenterToAffect, ConsequentialConstants.GroundPlayerCanMove)
        {
        }
