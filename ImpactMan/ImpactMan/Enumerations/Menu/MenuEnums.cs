using ImpactMan.Constants.Graphics;

namespace ImpactMan.Enumerations.Menu
{
    public enum Menu
    {
        MainMenuWidth = GraphicsConstants.PreferredBufferWidth,
        MainMenuHeight = GraphicsConstants.PreferredBufferHeight - 50,

        MainMenuItemWidth = 400,
        MainMenuItemHeight = 50,

        MainMenuPaddingTop = 100,
        MainMenuPaddingLeft = 450,


        NewGameMenuWidth = 450,
        NewGameMenuHeight = 450,

        NewGameMenuItemWidth = 150,
        NewGameMenuItemHeight = 50,

        NewGameMenuPaddingTop = 155,
        NewGameMenuPaddingLeft = 580,


        LoginMenuWidth = GraphicsConstants.PreferredBufferWidth,
        LoginMenuHeight = GraphicsConstants.PreferredBufferHeight - 50,

        LoginMenuItemWidth = 100,
        LoginMenuItemHeight = 30,

        LoginMenuPaddingTop = 260,
        LoginMenuPaddingLeft = 590,


        RegisterMenuWidth = GraphicsConstants.PreferredBufferWidth,
        RegisterMenuHeight = GraphicsConstants.PreferredBufferHeight - 50,

        RegisterMenuItemWidth = 100,
        RegisterMenuItemHeight = 30,

        RegisterMenuPaddingTop = 300,
        RegisterMenuPaddingLeft = 592,


        SettingsMenuWidth = GraphicsConstants.PreferredBufferWidth,
        SettingsMenuHeight = GraphicsConstants.PreferredBufferHeight - 50,

        SettingsMenuItemWidth = 400,
        SettingsMenuItemHeight = 50,

        SettingsMenuPaddingTop = 100,
        SettingsMenuPaddingLeft = 450,

        HighScoresMenuWidth = GraphicsConstants.PreferredBufferWidth,
        HighScoresMenuHeight = GraphicsConstants.PreferredBufferHeight - 50,

        HighScoresMenuItemWidth = 100,
        HighScoresMenuItemHeight = 30,

        HighScoresMenuPaddingTop = 670,
        HighScoresMenuPaddingLeft = 55,
    }   
}
